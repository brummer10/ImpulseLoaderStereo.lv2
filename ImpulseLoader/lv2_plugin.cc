...
void draw_waveview(void *w_, void* user_data) {
    ...
    if (!metrics.visible) return;
    int half_height_t = height_t * 0.5;
    int quarter_height_t = half_height_t * 0.5;
    int z = wave_view->size * 0.5;
    ...
    use_fg_color_scheme(w, NORMAL_);
    cairo_line_to(w->crb, width_t, quarter_height_t);
    cairo_stroke(w->crb);

    cairo_move_to(w->crb,2,quarter_height_t+ half_height_t);
    use_fg_color_scheme(w, NORMAL_);
    cairo_line_to(w->crb, width_t, quarter_height_t +half_height_t);
    cairo_stroke(w->crb);
    ...
    cairo_line_to(w->crb, width_t, quarter_height_t);
    cairo_line_to(w->crb, 2, quarter_height_t);
